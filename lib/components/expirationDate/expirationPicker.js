...
import { Input, Row, Col, Select, Typography } from '@';
...
const ExpirationPicker = React.memo(({ getTime, isModal, expDateTime }) => {
  ...
  const handleInputChange = (e) => {
    e.preventDefault();
    const inputtedValue = e.currentTarget.value.trim();
  ...
  return (
    <div className={styles.wrap}>
      <Input.Group>
        <Row gutter={8}>
          <Col span={5}>
            <Input onChange={handleInputChange} value={number} defaultValue="1" />
          </Col>
          <Col span={8}>
            <Select
      ...
      {!!number.length && <div className={styles.formattedText} data-testid="formatted-string">{formattedString}</div>}
    </div>
  );
});
...
ExpirationPicker.propTypes = {
  ...
  expDateTime: PropTypes.instanceOf(Date),
};
