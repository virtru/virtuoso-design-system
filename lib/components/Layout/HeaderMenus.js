...
function HeaderMenus({ children, menuAlignmentDefault }) {
  ...
  React.Children.forEach(children, (menu) => {
    ...
    menuComponentsByAlignments[menuAlignment].push(
      <div key={menu.key} className={classNames} aria-label={aria}>
        {nextMenu}
      </div>,
    );
  });
  ...
  return (
    <div className={styles.menuWrapper}>
      <div>{menuComponentsByAlignments[MENU_ALIGNMENT_LEFT]}</div>
      <div>{menuComponentsByAlignments[MENU_ALIGNMENT_RIGHT]}</div>
    </div>
  );
}
