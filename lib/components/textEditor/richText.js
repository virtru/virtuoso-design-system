...
import { createEditorState, editorStateToHTML } from '@/utils/richText';
...
const RichText = forwardRef(
  (
    ...
    const editorChangeHandler = useCallback(
      (newEditorState) => {
        ...
        if (editorStateProp) {
          ...
          if (html !== editorStateToHTML(editorStateProp)) {
            showWarningModal();
          }
        ...
        setEditorState((state) => {
          if (html !== editorStateToHTML(state)) {
    ...
    const editorHandleKeyCommandHandler = useCallback(
      (command) => {
        const newState = RichUtils.handleKeyCommand(editorStateProp || editorState, command);
      ...
      [editorChangeHandler, editorState, editorStateProp],
    );
    ...
    return (
      ...
      <div data-testid="richText" ref={editorWrapRef} className={styles.editor} onClick={focus}>
        <Editor
          ...
          editorState={editorStateProp || editorState}
